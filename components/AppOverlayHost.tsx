...
import { ArrowLeft, BookOpen, Clock3, Flame, Gift, Layers3, Snowflake, Table2, Trophy, type LucideIcon } from "lucide-react";
...
export function AppOverlayHost() {
  const { activeOverlay, closeOverlay, taskFlow, plans, proofs, deck, analysis } = useNextCardStore();
    ...
    <div className="fixed inset-0 z-[80] grid justify-items-center bg-[#fbf1ea]/96 backdrop-blur">
      <section className="flex h-dvh w-full max-w-[430px] flex-col overflow-hidden bg-[#fff8f1] px-4 pb-4 pt-[max(env(safe-area-inset-top),0.85rem)] shadow-soft">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto py-4">
          ...
          {activeOverlay.type === "deck-stack-detail" && <DeckStackReview decks={deck.decks} />}
          ...
          {activeOverlay.type === "proof-excel-review" && <ProofExcelReview decks={deck.decks} records={proofs.records} />}
...
function FrozenTodoReview({ records, decks }: { records: ProofRecord[]; decks: TaskDeck[] }) {
  ...
  return (
    <div className="grid gap-3">
      <ReviewFrame
        icon={Snowflake}
        title={`${frozen.length + frozenCards.length} 个冻结代办`}
        intro="冻结不是失败，是保存上下文。这里集中展示需要稍后恢复的卡片和它们的下一步。"
        records={frozen.slice(0, 5)}
      />
      <OverlaySection title="冻结卡片">
        {frozenCards.length > 0 ? (
          frozenCards.map(({ deck, card }) => (
            <MiniLine key={card.id} title={card.title} detail={`${deck.coverTitle} · ${card.cardBackNote}`} />
          ))
        ) : (
...
function BurnFailedReview({ records, decks }: { records: ProofRecord[]; decks: TaskDeck[] }) {
  ...
  return (
    <div className="grid gap-3">
      <ReviewFrame
        icon={Flame}
        title={`${burning.length + riskyCards.length} 条燃烧风险`}
        intro="燃烧代表最后可行动窗口正在变窄。完成会留下燃烧完成记录，未完成会保留为风险卡，方便下次提前安排。"
        records={burning.slice(0, 5)}
      />
      <OverlaySection title="燃烧失败/风险卡片">
        {riskyCards.length > 0 ? (
          riskyCards.map(({ deck, card }) => (
            <MiniLine key={card.id} title={card.title} detail={`${deck.coverTitle} · ${card.urgencyStage} · ${card.status}`} />
          ))
        ) : (
...
function DeckStackReview({ decks }: { decks: TaskDeck[] }) {
  ...
  return (
    <div className="grid gap-3">
      <OverlayCard icon={Layers3} title={`${unfinished.length} 张未完成卡`}>
        这里是 deck 侧边卡堆的完整解释：只展示还需要行动的卡，完成过的卡片会转移到 proof，不再占用专注视图。
      </OverlayCard>
      <OverlaySection title="未完成详情">
        {unfinished.length > 0 ? (
          unfinished.map(({ deck, card }) => (
            <MiniLine key={card.id} title={card.title} detail={`${deck.coverTitle} · ${card.status} · ${card.urgencyStage}`} />
          ))
        ) : (
          <p className="text-sm leading-6 text-ink/64">当前没有未完成卡片。</p>
        )}
      </OverlaySection>
    </div>
  );
...
function ProofExcelReview({ decks, records }: { decks: TaskDeck[]; records: ProofRecord[] }) {
    ...
    <div className="grid gap-3">
      ...
      <OverlaySection title="Excel-like 完成度表">
        <div className="overflow-x-auto">
          <div className="min-w-[24rem] overflow-hidden rounded-[1rem] border border-ink/10 bg-white/70">
            <div className="grid grid-cols-[1.35fr_0.72fr_0.72fr_0.9fr] bg-ink px-3 py-2 text-[0.66rem] font-semibold uppercase tracking-[0.08em] text-white/78">
              <span>card</span>
              <span>status</span>
              <span>done</span>
              <span>tone</span>
            ...
            {rows.map(({ deck, card }) => (
              <ExcelRow key={card.id} deck={deck} card={card} />
            ))}
            {rows.length === 0 && (
...
function ExcelRow({ deck, card }: { deck: TaskDeck; card: TaskCard }) {
  const tone = getExcelTone(card);
  const progress = getCardProgress(card);

  return (
    <div className="grid grid-cols-[1.35fr_0.72fr_0.72fr_0.9fr] border-t border-ink/8 px-3 py-2 text-xs text-ink/72">
      <div className="min-w-0">
        <div className="truncate font-semibold text-ink">{card.title}</div>
        <div className="mt-0.5 truncate text-[0.66rem] text-ink/42">{deck.coverTitle}</div>
      </div>
      <span className="self-center truncate">{card.status}</span>
      <span className="self-center font-semibold">{progress}%</span>
      <span className={`self-center rounded-full px-2 py-1 text-center text-[0.66rem] font-semibold ${tone.className}`}>
        {tone.label}
      </span>
    </div>
  );
...
function getExcelTone(card: TaskCard) {
  if (card.status === "completed" || card.status === "rewarded") {
    return { label: "完成", className: "bg-emerald-700 text-white" };
  }

  if (card.status === "frozen" || card.damageEffect === "freeze") {
    return { label: "冻结", className: "bg-sky-100 text-sky-900" };
  }

  if (card.urgencyStage === "burning" || card.urgencyStage === "expired" || card.damageEffect === "burn") {
    return { label: "燃烧", className: "bg-orange-100 text-orange-900" };
  }

  if (card.status === "active") {
    return { label: "进行中", className: "bg-[#fff4cc] text-ink" };
  }

  return { label: "待办", className: "bg-ink/[0.055] text-ink/58" };
}

...
export function CompactPlanCatalog({
  ...
  const subtitle = planSummary ?? selectedOption?.summary ?? selectedPlanName ?? `${groups.length} 个阶段 · ${total} 张卡`;

  return (
    <section className="relative z-10 flex h-full max-h-full min-h-0 flex-col overflow-hidden rounded-[1.55rem] border border-ink/10 bg-[#fffaf4]/86 p-4 text-ink shadow-[0_18px_46px_rgba(31,41,35,0.08)] backdrop-blur-xl">
      <div className="pointer-events-none absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent via-white to-transparent" aria-hidden />
      <header className="relative z-10 shrink-0">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[0.64rem] font-semibold uppercase tracking-[0.18em] text-fern/70">任务计划目录</div>
            <h2 className="mt-2 font-editorial text-[1.9rem] leading-[1.02] text-ink">{deck.coverTitle}</h2>
            <p className="mt-3 line-clamp-2 text-[0.82rem] leading-6 text-ink/58">{subtitle}</p>
          </div>
        ...
        <div className="mt-5">
          <div className="flex items-center justify-between gap-3 text-[0.68rem] font-semibold text-ink/42">
            <span>{groups.length} 个阶段 · {total} 张卡</span>
      ...
      <div className="relative z-10 mt-5 min-h-0 flex-1 overflow-y-auto pr-0.5">
        <div className="divide-y divide-ink/[0.075] border-y border-ink/[0.075]">
...
function CatalogGroupRow({ group, index }: { group: CatalogGroup; index: number }) {
  ...
  return (
    <div className={`relative grid min-h-[58px] grid-cols-[1.55rem_minmax(0,1fr)_4.5rem] items-center gap-2 px-1 py-3 ${tone.row}`}>
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate text-[0.92rem] font-semibold leading-5 text-ink">{group.title}</span>
        </div>
        <div className="mt-0.5 truncate text-[0.68rem] font-medium text-ink/44">
          包含 {group.cards.length} 张卡 · {tone.label}
        </div>
      </div>
      <span className="justify-self-end text-right text-[0.68rem] font-semibold text-ink/42">
        {minutes}m
      </span>
    </div>
  );
...
function getGroupTone(status: CatalogGroup["status"]) {
  ...
  if (status === "frozen") {
    return {
      label: "冰冻任务",
      row: "bg-[linear-gradient(90deg,rgba(240,251,255,0.64),transparent)]",
  ...
  if (status === "active") {
    return {
      label: "当前阶段",
      row: "bg-[linear-gradient(90deg,rgba(6,63,39,0.055),transparent)]",

...
Next Card now has a backend service layer for the core production behaviors: structured Plan Mode, import coverage review, priority scheduling, freeze return, worker ticks, and pluggable backend ports.
...
Real planning services must normalize provider output into the existing domain shapes from `lib/types.ts`:

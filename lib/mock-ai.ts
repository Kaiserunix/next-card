...
export function mockGeneratePlanOptions(analysis: AnalysisResult): PlanOption[] {
  ...
  return [
    attachAgentPolicy({
      ...
      name: "方案一",
      style: "urgent",
      summary: `先保护最小可行动作，适合时间紧或已经接近最佳窗口时使用。由${getAgentProfile(planAgents[0]).name}控制节奏。`,
    ...
    attachAgentPolicy({
      ...
      name: "方案二",
      style: "balanced",
      summary: `速度和完整度平衡，适合正常推进。由${getAgentProfile(planAgents[1]).name}维持节奏。`,
    ...
    attachAgentPolicy({
      ...
      name: "方案三",
      style: "gentle",
      summary: `低压力版本，允许中途冻结。由${getAgentProfile(planAgents[2]).name}降低重启阻力。`,
